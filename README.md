...
- Database: PostgreSQL (optional). Falls back to in-memory mode when DATABASE_URL is not set.
- Auth: bcryptjs + JWT
...
Backend running in local persistent mode (file storage).

This is expected for local/demo mode when DATABASE_URL is not set.
